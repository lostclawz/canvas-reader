...
document.getElementById('nav-search').addEventListener('keyup', function (event) {
  ...
  if (!search) {
    ...
    document.querySelectorAll('nav > ul > li:not(.level-hide)').forEach((elem) => {
    ...
    document.querySelectorAll('nav > ul').forEach((elem) => {
      elem.style.display = 'block';
    });
    //hide all results
    document.querySelectorAll('nav > ul > li > ul li').forEach((elem) => {
    ...
    document.querySelectorAll('nav > ul.collapse_top').forEach((parent) => {
      var countVisible = 0;
      parent.querySelectorAll('li').forEach((elem) => {
        if (elem.style.display !== 'none') {
          countVisible++;
        }
      });
